...
class MTools
{
  public:
    ...
    unsigned short factorV   = MPrj::factor_v_default;   // Коэффициент преобразования в милливольты
    unsigned short smoothV   = MPrj::smooth_v_default;   // Коэффициент фильтрации
             short shiftV    = MPrj::shift_v_default;    // Приборное смещение в милливольтах

    unsigned short factorI   = MPrj::factor_i_default;   // Коэффициент преобразования в миллиамперы
    unsigned short smoothI   = MPrj::smooth_i_default;   // Коэффициент фильтрации
             short shiftI    = MPrj::shift_i_default;    // Приборное смещение в миллиамперах
...
//    void txDiscurrentAdj(short spD);                        // 0x27 Регулировка тока разряда
  ...
  private:
    ...
    bool sync = false;                 // Флаг синхронизации параметров
