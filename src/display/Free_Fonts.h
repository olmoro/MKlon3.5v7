/*
Прикрепите этот заголовочный файл к своему эскизу, чтобы использовать бесплатные шрифты GFX. Вы можете писать
эскизы и без этого, но это облегчает обращение к ним.

При этом вызываются все шрифты, но они загружаются только в том случае, если вы действительно
используете их в своем эскизе.

Никаких изменений в этом заголовочном файле не требуется, если только новые шрифты не будут добавлены в папку
библиотеки "Fonts/GFX FF".

Чтобы сэкономить много времени при вводе длинных названий, на каждый шрифт можно легко ссылаться в
эскизе тремя способами, либо с помощью:

    1. Имя файла шрифта с & впереди, например &FreeSansBoldOblique24pt7b
       примером является:

       tft.setFreeFont(&FreeSansBoldOblique24pt7b);

    2. FF# где # - это число, определенное путем просмотра приведенного ниже списка
       примером является:

       tft.setFreeFont(FF32);

    3. Сокращение от имени файла. Посмотрите на приведенный ниже список, чтобы увидеть
       используемые сокращения, например:
 ...
 Установив для шрифта значение NULL, вы выберете шрифт GLCD:

      tft.setFreeFont(NULL); // Set font to GLCD
*/
#define LOAD_GFXFF

#ifdef LOAD_GFXFF // Включайте шрифты только в том случае, если LOAD_GFX FF определен в User_Setup.h

// Используйте их при печати или рисовании текстовых сообщений жидкокристаллическими шрифтами и шрифтами с высокой скоростью рендеринга
#define GFXFF 1
#define GLCD  0
#define FONT2 2
#define FONT4 4
...
// Используйте следующее при вызове setFont()
//
// Зарезервировано для шрифта GLCD // FF0
//

#define TT1 &TomThumb
// каждый символ имеет одинаковую ширину
#define FM9 &FreeMono9pt7b    // 18
#define FM12 &FreeMono12pt7b  // 24 px
#define FM18 &FreeMono18pt7b  // 36
#define FM24 &FreeMono24pt7b  // 48
...
#define FSS9 &FreeSans9pt7b    // 22 px
#define FSS12 &FreeSans12pt7b  // 29 px (23px)
#define FSS18 &FreeSans18pt7b  // 44 px
#define FSS24 &FreeSans24pt7b  // 58 px
...
#define FS9 &FreeSerif9pt7b    // 22 px
#define FS12 &FreeSerif12pt7b  // 29 px
#define FS18 &FreeSerif18pt7b  // 44 px
#define FS24 &FreeSerif24pt7b  // 58 px
...
// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
// Теперь мы определяем "s" пробных версий для удобства печати названия шрифта, так что:
//   tft.println(sFF5);
// will print
//   Mono bold 9
// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
...
#define sFF16 "Mono bold oblique 24" // Полнотекстовая строка слишком велика для экрана шириной 480 пикселей
...
#else // LOAD_GFX FF не определен, поэтому установите значения по умолчанию для предотвращения сообщений об ошибках

/* >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
 Бесплатные шрифты не загружены в User_Setup.h, поэтому мы должны определить все как шрифт 1
, чтобы предотвратить сообщения об ошибках компилятора
 >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
*/
#define GFXFF 1
#define GLCD  1
